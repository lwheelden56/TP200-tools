# Script to automate Toast TP200/TP200W printer tests
# Untested
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if re.search(r'<TITLE>Ethernet port configuration</TITLE>', response.text):
                print(f"{ip}: Sending Test Command...")
                reboot_device(ip)
            else:
...
def reboot_device(ip):
    try:
        response = requests.post(f"http://{ip}/cgi-bin/ipcfg", data={'page_p2': 'Print Test Page'}, timeout=0.1)
        print(f"Test command sent to {ip}.")
    except requests.RequestException as e:
        #print(f"Error sending test command to {ip}: {e}")
        pass
